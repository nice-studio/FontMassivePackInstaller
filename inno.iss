...
[Run]
Filename: {app}\FonTemp.EXE; Description: Запустить Менеджер коллекций шрифтов; Flags: postinstall nowait skipifsilent runascurrentuser unchecked
Filename: {app}\FontMassive.EXE; Description: Запустить FontMassive; Flags: postinstall nowait skipifsilent runminimized runascurrentuser unchecked
...
var
	...
	URL, ObName: String;
begin
	URL := ExpandConstant('{#AppUrl}');
	ObName := TLabel(Sender).Name;
	if ObName = 'GITHUB' then
	begin
		URL := ExpandConstant('{#GitReleace}');
	end;
	if ObName = 'PROJECTSOFT' then
	begin
		URL := ExpandConstant('{#InstallBuilderUrl}');
	end;
...
begin
	...
	begin
		...
		Parent := MainForm;
		Alignment := taRightJustify;
		Top := MainForm.Height - Height - 11;
		Left := MainForm.Width - Width - 21;
		OnClick := @labelHandle;
		Name := 'GITHUB';
		Hint := ExpandConstant('{cm:HintLabel1_0}') + #13#10 + ExpandConstant('{cm:HintLabel1_1}');
		ShowHint := True;
	end;

	LabelCaption := TLabel.Create(WizardForm);
	with LabelCaption do
	begin
		Caption := ExpandConstant('{cm:CaptionLabel1}') + ': ';
		AutoSize := True;
		Left := 8;
		Top := WizardForm.NextButton.Height + WizardForm.NextButton.Top - Height;
	...
	begin
		...
		AutoSize := True;
		Left := 7;
		Top := WizardForm.NextButton.Height + WizardForm.NextButton.Top - Height - 1;
		Parent := WizardForm.NextButton.Parent;
		Visible:= True;
	end;
	...
	with TLabel.Create(WizardForm) do
	begin
		Caption := ExpandConstant('{#InstallBuilder}');
		AutoSize := True;
		Left := LabelCaption.Width + LabelCaption.Left;
		Top := WizardForm.NextButton.Height + WizardForm.NextButton.Top - Height;
		Parent := WizardForm.NextButton.Parent;
		Visible:= True;
		Transparent := True;
	...
	begin
		Name := 'PROJECTSOFT';
		Hint := ExpandConstant('{cm:HintLabel2_0}');
		ShowHint := True;
		Caption := ExpandConstant('{#InstallBuilder}');
		AutoSize := True;
		Left := LabelCaption.Width + LabelCaption.Left - 1;
		Top := WizardForm.NextButton.Height + WizardForm.NextButton.Top - Height - 1;
		OnClick := @labelHandle;
		Parent := WizardForm.NextButton.Parent;
		Visible:= True;
	...
	begin
		Name := 'FONTMASSIVE';
		Hint := ExpandConstant('{cm:HintLabel3_0}');
		ShowHint := True;
		Caption := ExpandConstant('{#AppUrl}');
		AutoSize := True;
		Left := 7;
